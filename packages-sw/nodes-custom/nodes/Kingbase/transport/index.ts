...
import type {
	ConnectionsData,
	KbpConnectionParameters,
	KingbaseNodeCredentials,
	KingbaseNodeOptions,
} from '../v2/helpers/interfaces';

const getKingbaseConfig = (
	credentials: KingbaseNodeCredentials,
	options: KingbaseNodeOptions = {},
) => {
	const dbConfig: KbpConnectionParameters = {
...
export async function configureKingbase(
	...
	const fallBackHandler = async (abortController: AbortController) => {
		...
		const dbConfig = getKingbaseConfig(credentials, options);
		...
		if (!credentials.sshTunnel) {
			...
			abortController.signal.addEventListener('abort', async () => {
				this.logger.debug('configureKingbase: Got abort signal, closing kb connection.');
				try {
					if (!db.$pool.ended) await db.$pool.end();
				} catch (error) {
					this.logger.error('configureKingbase: Encountered error while closing the pool.', {
						error,
					});
					throw error;
				}
