		...
		if (['>', '<', '>=', '<='].includes(clause.condition)) {
			...
			if (Number.isNaN(value)) {
				throw new NodeOperationError(
					node,
					`Operator in entry ${index + 1} of 'Select Rows' works with numbers, but value ${clause.value
					} is not a number`,
					{
						itemIndex,
					},
...
export async function uniqueColumns(db: KbpDatabase, table: string, schema = 'public') {
...
export async function getEnums(db: KbpDatabase): Promise<Map<string, string[]>> {
...
export const configureTableSchemaUpdater = (initialSchema: string, initialTable: string) => {
	...
	return async (db: KbpDatabase, tableSchema: ColumnInfo[], schema: string, table: string) => {
...
export const runQueriesAndHandleErrors = async (
	...
	queries: QueryWithValues[],
	nodeOptions: KingbaseNodeOptions,
