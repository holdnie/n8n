...
import type { KingbaseNodeCredentials } from '../helpers/interfaces';

export async function schemaSearch(this: ILoadOptionsFunctions): Promise<INodeListSearchResult> {
	const credentials = await this.getCredentials<KingbaseNodeCredentials>('kingbase');
	const options = { nodeVersion: this.getNode().typeVersion };

	const { db } = await configureKingbase.call(this, credentials, options);
